...
//           homeLoan.kf_name &&
//           homeLoan.kf_name.toUpperCase().includes(searchQuery.toUpperCase())
//         );

//         const filteredPersonalLoans = personalLoans.filter((personalLoan) =>
//           personalLoan.kf_name &&
//           personalLoan.kf_name.toUpperCase().includes(searchQuery.toUpperCase())
//         );

//         setHomeLoanContacts(filteredHomeLoans);
//         setPersonalLoanContacts(filteredPersonalLoans);
//         console.log("Response Personal Loans:", responsePersonalLoans.data);
//     console.log("Response Loan Applications:", responseLoanApplications.data);
//       } catch (error) {
//         console.error("Error during data fetch error:", error);
//       } finally {
//         setLoading(false);
...
//   const handleDynamicDashboard = () => {
//  try {
//   navigate('DynamicDashboardScreen');
// } catch (error) {
//   console.error('Error navigating to DynamicDashboardScreen:', error);
// }
...
//   };
// // Update displayed loans based on showAllLoans state
// const displayedHomeLoans = showAllLoans ? HomeLoanRecords : HomeLoanRecords.slice(0, 3);
// const displayedPersonalLoans = showAllLoans ? PersonalLoanRecords : PersonalLoanRecords.slice(0, 3);
...
//             <ScrollView contentContainerStyle={{ width: "100%", paddingTop: 0 }}>
//               <Text style={{ fontSize: 18, fontWeight: 'bold', marginBottom: 10 }}>Home Loans</Text>
//               {HomeLoanRecords.length > 0 ? (
//                 HomeLoanRecords.map((homeLoan, index) => (
//                   <HomeLoanCard
//                     key={index}
//                     loanApplication={homeLoan}
//                     onPress={() => navigateToHomeDetails(homeLoan)}
...
//                 ))
//               ) : (
//                 <Text style={{ fontSize: 16, fontStyle: 'italic', color: 'grey', textAlign: 'center' }}>
//                   No Home Loans found.
//                 </Text>
//               )}

//               <Text style={{ fontSize: 18, fontWeight: 'bold', marginTop: 20, marginBottom: 10 }}>Personal Loans</Text>
//               {PersonalLoanRecords.length > 0 ? (
//                 PersonalLoanRecords.map((personalLoan, index) => (
//                   <PersonalLoanCard
//                     key={index}
//                     personalLoan={personalLoan}
//                     onPress={() => navigateToPersonalDetails(personalLoan)}
