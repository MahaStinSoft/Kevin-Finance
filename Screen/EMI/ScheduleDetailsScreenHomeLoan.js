...
//     useEffect(() => {
//         // Fetch other charges data for the current application number when the component mounts
    ...
    const createNoteWithFile = async () => {
        try {
            if (!isPaid && (scheduleItem.month === 1 || (scheduleItem.month > 1 && loanDetails.amortizationSchedule[scheduleItem.month - 2]?.paid))) {
                ...
                const annotation = {
                    subject: subject,
                    'objectid_kf_loanapplication@odata.bind': `/kf_loanapplications(${relatedRecordId})`,
                };
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={{ padding: 20 }}>
                ...
                <Text>New EMI Payment: {newEMIPayment}</Text>
                <ButtonComponent 
                title="Create Note with File" 
                onPress={createNoteWithFile} 
                disabled={isPaid || isNoteCreationDisabled} 
                />
            </View>
        </View>
    );
