...
useEffect(() => {
    ...
    useEffect(() => {
        console.log('EMI Amount:', scheduleItem.emiAmount);
    ...
    const handleOtherChargesChange = async (text) => {
        try {
            // Update other charges for the specific month in the amortization schedule
            const updatedAmortizationSchedule = loanDetails.amortizationSchedule.map(item => {
                if (item.month === scheduleItem.month) {
                    return { ...item, otherCharges: text };
                }
                return item;
            });
    
            // Update loanDetails with the new amortization schedule
            const updatedLoanDetails = { ...loanDetails, amortizationSchedule: updatedAmortizationSchedule };
    
            // Save the updated loanDetails to AsyncStorage
            await AsyncStorage.setItem('loanDetails', JSON.stringify(updatedLoanDetails));
    
            // Save other charges to AsyncStorage for the specific month
            await AsyncStorage.setItem(`otherCharges_${scheduleItem.month}`, text);
    
            // Update state with the new other charges
            setOtherCharges(text);
        } catch (error) {
            console.error('Error saving other charges:', error);
        }
                    ...
                    [
                        {
                            text: 'OK',
                            onPress: () => {
                                // Navigate to AmortizationScreenHome with updated payment status
